...
const CreateTaskForm = () => {
    ...
    useEffect(() => {
        if (id) {
            const getTask = async () => {
                try {
                    const response = await axios.get(`http://localhost:3000/api/tasklist/${id}`, {
                        headers: {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            if (id) {
                const response = await axios.put(`http://localhost:3000/api/updatetask/${id}`, taskDetails, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
                ...
                const response = await axios.post('http://localhost:3000/api/createtask', taskDetails, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
        ...
        <div>
            ...
            <form onSubmit={handleSubmit} className='bg-indigo-50 w-[400px] mx-auto mt-10 flex flex-col gap-5 px-5 py-16 rounded-2xl'>
                ...
                <fieldset className='flex flex-col gap-2'>
                    ...
                    <input type="date" name="scheduletime" id="scheduletime" className='rounded-xl p-2' placeholder='Schedule Time' value={taskData.scheduletime} onChange={handleInputChange} />
                </fieldset>
                <fieldset className='flex flex-col gap-2'>
